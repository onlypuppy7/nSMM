...
console.BACKGROUND_COLOR = {0, 0, 0, 0.6}
...
_G.print = function(...)
  normal_print(...) -- Keep original behavior

  local args = {...}
  local parts = {}

  for i = 1, select("#", ...) do
    local ok, str = pcall(tostring, args[i])
    if ok then
      table.insert(parts, str)
    else
      table.insert(parts, "<tostring error>")
    end
  end
...
function console.keypressed(key, scancode, isrepeat)
  ...
  elseif key == 'escape' then enabled=false
