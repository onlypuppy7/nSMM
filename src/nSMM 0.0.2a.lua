...
--X movement
    if arrowLeft==1 or arrowRight==1 then
        if math.abs(mario.velocity.x)<12 then
            if math.abs(mario.velocity.x)<2.7 then
...
--Y handling
    if gravityCheck(mario.pos.x,mario.pos.y,1) and not (mario.velocity.y>0.1) then --gravity occurs!
